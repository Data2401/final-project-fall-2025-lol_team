...
## TMR Tbl -TY
Created a new table called 'TMM_Tbl' to join Team Match Table and Match Table by MatchFk and MatchId.
...
med2 <- ggplot(m, aes(x = median_TurretDmgDealt, y = Lane, fill = Lane)) + geom_col()
med2

max2 <- ggplot(m, aes(x = max_TurretDmgDealt, y = Lane, fill = Lane)) + geom_col()
max2
```
...
mean3 <- ggplot(m, aes(x = avg_DmgTaken, y = Lane, fill = Lane)) + geom_col()
mean3  

med3 <- ggplot(m, aes(x = median_DmgTaken, y = Lane, fill = Lane)) + geom_col()
med3

max3 <- ggplot(m, aes(x = max_DmgTaken, y = Lane, fill = Lane)) + geom_col()
max3
```
...
mean4 <-ggplot(m, aes(x = avg_DmgDealt, y = Lane, fill = Lane)) + geom_col()
mean4

med4 <- ggplot(m, aes(x = median_DmgDealt, y = Lane, fill = Lane)) + geom_col()
med4

max4 <- ggplot(m, aes(x = max_DmgDealt, y = Lane, fill = Lane)) + geom_col()
max4
```
...
mean5 <-ggplot(m, aes(x = avg_TotalGold, y = Lane, fill = Lane)) + geom_col()
mean5

med5 <- ggplot(m, aes(x = median_TotalGold, y = Lane, fill = Lane)) + geom_col()
med5

max5 <- ggplot(m, aes(x = max_TotalGold, y = Lane, fill = Lane)) + geom_col()
max5
```

Combining multiple plots for all the averages
```{r}
grid.arrange(mean1, mean2, mean3, mean4, mean5, nrow=5)
...
ggplotly(ggplot(MSI_Tbl %>% 
                  group_by(Lane) %>% 
                  count(Lane, wt = kills) %>% 
                  rename(kills = n), aes(x = factor(Lane,
                                                    levels = c("Top", "Jungle", "Middle", "Bottom", "Support", "None")), y = kills, fill = Lane, text = paste("Count:", after_stat(y)))) + geom_col() + labs(title = "Kills vs Lane", x = "Lane", y = "kills"), tooltip= "text") 
```
